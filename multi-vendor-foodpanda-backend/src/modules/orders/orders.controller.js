    ...
    try {
                    ...
                    if (item.menuItem && item.menuItem.recipeIngredients) {
                        for (const recipeIngredient of item.menuItem.recipeIngredients) {
                            ...
                            if (updatedIng.currentStock < updatedIng.lowStockThreshold) {
                                ...
                                NotificationService.sendTenantNotification(updatedIng.tenantId, `Low Stock Alert: ${updatedIng.name}`, `You are running low on ${updatedIng.name}. Current stock is ${updatedIng.currentStock} ${updatedIng.unit}, which is below your threshold of ${updatedIng.lowStockThreshold} ${updatedIng.unit}.`);
                            }
                        }
                    }
                }
        ...
        if (status === 'READY') {
            ...
            if (availableRider) {
                ...
                NotificationService.sendCustomerNotification(assignedOrder.customerPhone, 'Rider Assigned', `Great news! A rider has been assigned and is heading to the restaurant to pick up your order.`);
        ...
        NotificationService.sendCustomerNotification(updatedOrder.customerPhone, `Order Update: ${status}`, `Your order #${updatedOrder.id.slice(0, 6).toUpperCase()} is now ${status.replace('_', ' ')}!`);
